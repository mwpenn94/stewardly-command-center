...
const PLATFORMS = [
  {
    ...
    fields: [
      { key: "API Key", required: false, hint: "Recommended: Use a Private Integration Token (PIT) from Settings → Private Integrations. It's a non-expiring, scope-restricted JWT. Alternatively, use a legacy API Key from Settings → Business Profile." },
      { key: "JWT Token", required: false, hint: "Development only: Paste a session JWT from browser localStorage. Expires ~1 hour. Use PIT above for production." },
      { key: "Location ID", required: false, hint: "Auto-extracted from JWT/PIT, or enter manually. Found in Settings → Business Profile → Location ID." },
      { key: "Company ID", required: false, hint: "Found in Settings → Company → Company ID (optional, auto-extracted from JWT)" },
    ],
    setupGuide: [
      "Log in to GoHighLevel at app.gohighlevel.com",
      "Go to Settings → Private Integrations → Create new Integration (recommended). Name it 'Stewardly Command Center'.",
      "Select scopes: contacts (read+write), conversations (read+write), conversations/message (read+write), locations (read), locations/customFields (read+write), locations/tags (read+write), users (read)",
      "Copy the generated token and paste it in the API Key field below. Location ID is auto-extracted.",
    ],
    color: "text-blue-400",
